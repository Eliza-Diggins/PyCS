...
def issue_memory_waring(nml_config):
    ...
    box_size = (float(nml_config["AMR_PARAMS"]["boxlen"][0])*float(nml_config["UNITS_PARAMS"]["units_length"][0]))*pyn.units.Unit("cm")
    ...
    ic_loc = os.path.join(nml_config["INIT_PARAMS"]["initfile(1)"][0].replace("'",""),nml_config["DICE_PARAMS"]["ic_file"][0].replace("'",""))
    ...
    if mem_mode == "tot":
        ...
        mem_val_string = "%s GB/CPU (%s GB / 16 Core Node)" % (np.round(mem_val, decimals=2),np.round(mem_val*16, decimals=2))
    ...
    if mem_mode == "tot":
        ...
        print("[%s] = %s (%s %% Usage)\n\t{Dark Matter: %s (%s %% Usage), Baryonic: %s (%s %% Usage)}" % (
            ...
            Fore.RED + Style.BRIGHT + str(np.round(100*sum(list(ic_parts)) / float(npart),decimals=3)) + Style.RESET_ALL,
