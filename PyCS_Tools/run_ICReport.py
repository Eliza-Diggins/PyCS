...
########################################################################################################################
    ...
    if not initial_condition_file in iclog: # we don't have an entry.
        ...
        log_data = iclog[initial_condition_file]
    ...
    report_data["General"]["N Particles"] = np.format_float_scientific(len(snapshot),precision=3) # recording the total number of particles
    ...
    report_data["General"]["Mass"] = str(np.format_float_scientific(np.sum(snapshot["mass"].in_units(CONFIG["units"]["default_mass_unit"])),precision=3))+ " %s"%CONFIG["units"]["default_mass_unit"]

    #- Giving family level general data -#
    for family in snapshot.families():
        report_data["General"][family.name] = {}
        report_data["General"][family.name]["N Particles"] = np.format_float_scientific(len(snapshot[family]),precision=3)
        ...
        report_data["General"][family.name]["Mass"] = str(np.format_float_scientific(
            np.sum(snapshot[family]["mass"].in_units(CONFIG["units"]["default_mass_unit"])),precision=3)) + " %s" % CONFIG["units"][
                                             "default_mass_unit"]
    ...
    if log_data["type"] == "cluster-binary": #the IC file has a binary collision.
        ...
        for subsnap,id,com in zip(binary_snapshots,["1","2"],binary_coms):
            ...
            report_data["Cluster %s"%id]["N Particles"] = np.format_float_scientific(len(subsnap),precision=3)  # recording the total number of particles
            ...
            report_data["Cluster %s"%id]["COM"] = [str(i) + " %s"%CONFIG["units"]["default_length_unit"] for i in list(com.in_units(CONFIG["units"]["default_length_unit"]))]
            report_data["Cluster %s"%id]["COM-Velocity"] = [str(i)+" %s"%CONFIG["units"]["default_velocity_unit"] for i in list(pyn.analysis.halo.center_of_mass_velocity(subsnap).in_units(CONFIG["units"]["default_velocity_unit"]))]
            ...
            for family in subsnap.families():
                ...
                report_data["Cluster %s"%id][family.name]["N Particles"] = np.format_float_scientific(len(subsnap[family]),precision=3)
                report_data["Cluster %s"%id][family.name]["Loadable_keys"] = [key for key in subsnap[family].loadable_keys()]
                report_data["Cluster %s"%id][family.name]["Mass"] = str(np.format_float_scientific(
                    np.sum(subsnap[family]["mass"].in_units(CONFIG["units"]["default_mass_unit"])),precision=3)) + " %s" % \
                                                    CONFIG["units"][
                                                        "default_mass_unit"]
    else:
        #- This isn't a binary -#
    ...
    ####################################################################################################################
    report_dir = os.path.join(CONFIG["system"]["directories"]["reports_directory"],initial_condition_file.split(".")[0])
