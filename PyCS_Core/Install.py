...
def file_select():
    ...
    while not CHECK:
        ...
        if os.path.exists(installation_directory) and os.path.isdir(installation_directory):
            ...
            chk = input(
                "%s:[%s]: The installation directory %s does not yet exist. [y] to create it. [n] to try again:" % (
                fdbg_string,
            ...
            if chk == "y":
                try:
                    ...
                    tmp = input(
                        "%s:[%s]: Failed to generate the installation directory %s. Press any key to try again." % (
                        fdbg_string,
    ...
    for directory in file["files"]:
        if directory != None:
            ...
            if file["files"][directory][
                "setting_name"] != None:  # the setting name is not NONE, we need to add to config.
                installation_config["system"]["directories"][file["files"][directory]["setting_name"]] = os.path.join(
                    location, directory)
...
if __name__ == '__main__':
    ...
    print("%sFound the installation configuration. Installing PyCS version %s." % (
    fdbg_string, Fore.CYAN + Style.BRIGHT + installation_config["system"]["version"] + Style.RESET_ALL))
    # --------------------------------------------------------#
    ...
    # --------------------------------------------------------#
    with open(os.path.join(installation_config["system"]["directories"]["bin_directory"], "configs", "CONFIG.ini"),
              "w") as file:
    ...
    for file in os.listdir(__install_config_file_path):
        if file != "CONFIG.ini" and file != "config.ini":
            ...
            shutil.copy(os.path.join(__install_config_file_path, file),
                        os.path.join(installation_config["system"]["directories"]["bin_directory"], "configs", file))
    ...
    if not os.path.exists(os.path.join(str(pt.Path(os.path.realpath(__file__)).parents[0]).replace(".py", ""), "tkt")):
        pt.Path.mkdir(
            pt.Path(os.path.join(str(pt.Path(os.path.realpath(__file__)).parents[0]).replace(".py", ""), "tkt")),
            parents=True)
    with open(os.path.join(str(pt.Path(os.path.realpath(__file__)).parents[0]).replace(".py", ""), "tkt",
                           "ticket.INSTALL_TICKET"), "w+") as file:
